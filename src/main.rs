const REAL_DATA: &'static str = "heightseven4two5
npskfdstpk2knsm
...
const PRACTICE_STRING: &'static str = "1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet";
...
fn main() {
    ...
    for line in REAL_DATA.split_whitespace() {
        // println!("{}", line);
        // line.rev();
        let first_digit_index = line.find(char::is_numeric).unwrap();
        let last_digit_index = line.rfind(char::is_numeric).unwrap();

        let first_digit = line.get(first_digit_index..first_digit_index + 1).unwrap();
        let last_digit = line.get(last_digit_index..last_digit_index + 1).unwrap();
        let sum = [first_digit, last_digit].join("").parse::<i32>().unwrap();

        total += sum;
        println!("{}", sum);
        // println!("first digit: {}", line.get[first_digit.or_else(0)..]);
        // println!("last digit: {}", line.get[last_digit.or_else(0)..]);
    }
    println!("total: {}", total);
}
